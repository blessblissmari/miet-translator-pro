...
function toSuper(s: string): string | null {
  let out = "";
  for (const ch of s) {
...
function replaceCommands(s: string): string {
  return s.replace(/\\([A-Za-z]+)(?![A-Za-z])/g, (m: string, name: string): string => {
...
function processDollarMath(s: string): string {
...
function applyTextFallbacks(s: string): string {
...
export function hasUnrenderedMath(text: string): boolean {
