...
export async function translateSlides({ apiKey, model, pages, concurrency = 4, onProgress }) {
  let done = 0;
  const results = await mapWithConcurrency(pages, concurrency, async (page) => {
    ...
    try {
      ...
      if (parsed && typeof parsed.title === "string") {
        const title = applyGlossaryPost(parsed.title.trim()).slice(0, 120);
        const bullets = Array.isArray(parsed.bullets)
          ? parsed.bullets.map(b => applyGlossaryPost(String(b).trim())).filter(Boolean).slice(0, 8)
          : [];
        const layout = bullets.length === 0 ? "section-title" : "title-text";
        plan = { title, bullets, layout };
      } else {
        // Fallback: take first non-empty line as title, next 5 as bullets
        const lines = (raw || "").split(/\n+/).map(l => l.trim()).filter(Boolean);
        const title = applyGlossaryPost(lines[0] || `Слайд ${page.index + 1}`).slice(0, 120);
        const bullets = lines.slice(1, 8).map(applyGlossaryPost);
